...
CREATE TABLE IF NOT EXISTS `bss_pagos`.`fase_proyecto` (
  ...
  CONSTRAINT `fk_faseproyecto`
    FOREIGN KEY (`idproyecto`)
    REFERENCES `bss_pagos`.`proyecto` (`idproyecto`),
  CONSTRAINT `fk_tipotra`
...
CREATE TABLE IF NOT EXISTS `bss_pagos`.`horas_trabajo` (
  ...
  INDEX `fk_fase` (`idfase` ASC) VISIBLE,
  CONSTRAINT `fk_fase`
    FOREIGN KEY (`idfase`)
...
CREATE TABLE IF NOT EXISTS `bss_pagos`.`pagos` (
  ...
  PRIMARY KEY (`idpago`),
  INDEX `fk_idhoras` (`idhorastrabajo` ASC) VISIBLE,
  CONSTRAINT `fk_idhoras`
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bss_pagos`.`pagos_emp` (`idhorastrabajo` INT, `fecha` INT, `hora_inicio` INT, `hora_final` INT, `hora_total` INT, `idfase` INT, `estado` INT, `tipo` INT, `precio` INT, `fase` INT, `idempleado` INT, `idtipotrabajo` INT, `empleado` INT, `telefono` INT, `proyecto` INT, `direccion` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bss_pagos`.`view_horastrabajo` (`idhorastrabajo` INT, `fecha` INT, `hora_inicio` INT, `hora_final` INT, `hora_total` INT, `idfase` INT, `estado` INT, `tipo` INT, `precio` INT, `fase` INT, `idempleado` INT, `idtipotrabajo` INT, `proyecto` INT, `direccion` INT, `empleado` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bss_pagos`.`view_pagos` (`idpago` INT, `idhorastrabajo` INT, `precio` INT, `cantidadhora` INT, `horasextra` INT, `subtotal` INT, `descuento` INT, `total` INT);
...
begin
select ht.*,  tt.tipo, tt.precio, f.nombre as fase, f.idempleado,f.idtipotrabajo,
p.nombre as proyecto, p.direccion
from horas_trabajo ht inner join fase_proyecto f on ht.idfase=f.idfase
...
where ht.estado='Pendiente' and e.idempleado=_idempleado;
end$$
...
case accion
when 'new' then
insert into horas_trabajo(fecha,hora_inicio,hora_total,idfase,estado)
values(_fecha,_hora_inicio,_hora_total,_idfase,_estado);
when 'update' then
update horas_trabajo set fecha=_fecha, hora_inicio=_hora_inicio, hora_final=_hora_final, hora_total=_hora_total,
idfase=_idfase, estado=_estado
...
case accion
when 'new' then
insert into pagos(idhorastrabajo,fecha,precio,cantidadhora,horasextra,subtotal,descuento,total)
values(_idhorastrabajo,curdate(),_precio,_cantidadhora,_horasextra,_subtotal,_descuento,_total);
when 'update' then
update pagos set idhorastrabajo=_idhorastrabajo,fecha=curdate(), precio=_precio, cantidadhora=_cantidadhora, horasextra=_horasextra,
...
USE `bss_pagos`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `bss_pagos`.`view_horastrabajo` AS select `ht`.`idhorastrabajo` AS `idhorastrabajo`,`ht`.`fecha` AS `fecha`,`ht`.`hora_inicio` AS `hora_inicio`,`ht`.`hora_final` AS `hora_final`,`ht`.`hora_total` AS `hora_total`,`ht`.`idfase` AS `idfase`,`ht`.`estado` AS `estado`,`tt`.`tipo` AS `tipo`,`tt`.`precio` AS `precio`,`f`.`nombre` AS `fase`,`f`.`idempleado` AS `idempleado`,`f`.`idtipotrabajo` AS `idtipotrabajo`,`p`.`nombre` AS `proyecto`,`p`.`direccion` AS `direccion`,concat(`e`.`nombre`,' ',`e`.`apellido`) AS `empleado` from ((((`bss_pagos`.`horas_trabajo` `ht` join `bss_pagos`.`fase_proyecto` `f` on((`ht`.`idfase` = `f`.`idfase`))) join `bss_pagos`.`tipo_trabajo` `tt` on((`f`.`idtipotrabajo` = `tt`.`idtrabajo`))) join `bss_pagos`.`proyecto` `p` on((`f`.`idproyecto` = `p`.`idproyecto`))) join `bss_pagos`.`empleado` `e` on((`f`.`idempleado` = `e`.`idempleado`))) where (`ht`.`estado` = 'Pendiente');
...
USE `bss_pagos`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `bss_pagos`.`view_pagos` AS select `bss_pagos`.`pagos`.`idpago` AS `idpago`,`bss_pagos`.`pagos`.`idhorastrabajo` AS `idhorastrabajo`,`bss_pagos`.`pagos`.`precio` AS `precio`,`bss_pagos`.`pagos`.`cantidadhora` AS `cantidadhora`,`bss_pagos`.`pagos`.`horasextra` AS `horasextra`,`bss_pagos`.`pagos`.`subtotal` AS `subtotal`,`bss_pagos`.`pagos`.`descuento` AS `descuento`,`bss_pagos`.`pagos`.`total` AS `total` from `bss_pagos`.`pagos`;
...
AFTER DELETE ON `bss_pagos`.`pagos`
FOR EACH ROW
begin
update horas_trabajo set estado="Pendiente" where idhorastrabajo=old.idhorastrabajo;
end$$
...
AFTER INSERT ON `bss_pagos`.`pagos`
FOR EACH ROW
begin
update horas_trabajo set estado="Cancelado" where idhorastrabajo=new.idhorastrabajo;
end$$
